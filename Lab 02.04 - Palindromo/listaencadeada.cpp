...
void ListaEncadeada::remove_consecutivos(){
  for (int i = 0 ; i< _num_elementos_inseridos-1 ; i++){
    int elemento = get_iesimo(i);
     for (int j = i+1 ; j <=_num_elementos_inseridos ; j++){
      int eleProx = get_iesimo(i+1);
      if (elemento == eleProx){
        remover_iesimo(i+1);
      }
    }
...
bool ListaEncadeada::checa_palindromo(){
  ...
  if(_num_elementos_inseridos%2 == 0){ //par
    int meio =( _num_elementos_inseridos / 2 ) - 1;
    ...
    palindromo = true;
    int mediana = _num_elementos_inseridos / 2 ;
